...
void executeCommand(char* args[], int numArgs, int numPipes){
	...
	if(numPipes == 0){
		...
		if(pid1 == 0){
			...
			dup2(fdwr1, 1);
			close(fdwr1);
			execlp(cmd1, args[0], args[1], NULL); //write fifo1
		}
		else{
			pid2 = fork();
			if(pid2 == 0){
				dup2(ogFd, 1);
				execlp(cmd2, args[3], FIFO1, NULL); // read fifo1	
			}
			else{
				waitpid(pid2, NULL, 0);
			}
		...
		if(pid1 == 0){
			...
			if(pid2 == 0){
				...
				dup2(fdwr2, 1);
				close(fdwr2);
				execlp(cmd2, args[3], FIFO1, NULL); // read fifo 1, write fifo 2
			}
			else{
				pid3 = fork();
				if(pid3 == 0){
...
int main(void){
	...
	while(1){
		...
		while(currToken){ 
			if(strcmp(currToken, "|") == 0) numPipes++;
		...
		else if(strcmp(argBuffer[0], "") != 0 && numArgs != 1 && numArgs != 2 && numArgs != 4 && numArgs != 6){
